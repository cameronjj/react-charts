...
//change format
function formatObservations(input) {
	var output = _.map(input, (d,i) => {
			var dict = { time: i, observations: d }
			return dict;
		})
	return output
}
...
export default class ProcessData extends React.Component {
  render(){
    ...
    let data1 = process(data)
		console.log(data1)
    return <SimpleBarChart dataKey={'observations'} color = {'#8884d8'} dataset={data1}/>
  }
}
